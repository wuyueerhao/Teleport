...
ajx_a = 'a:not(.widget_links a ,article section a,.social_links a,.header_logo a,.copyright a,#comments a,#wpadminbar a)'  , // a标签，自己添加排除规则
ajx_comt = 'comments' , // 整个评论区的id ,不加#
ajx_submit_form = '#comment_form' , // 提交按钮所在的表单
ajx_comtlist = '.commentlist' , // 评论列表id或class
ajx_comtpagenav = '.pagenav' , // 评论分页导航的id或class
ajx_comtpagenav_a = '.pagenav a' , // 评论分页导航的a标签
ajx_sform = '.widget_search form' , // 搜索表单form标签
ajx_skey = '#s' ; // 搜索表单input标签内的id或class
...
// 主页地址，用于下面的提交函数
var home_url = document.location.href.match(/https:\/\/([^\/]+)\//i)[0]; 
// 函数： 替换url，用于评论ajax提交
function replaceUrl(url, domain) {
    return url.replace(/https:\/\/([^\/]+)\//i, domain);
}
// 函数：从封装的Json获取
